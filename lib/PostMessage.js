/**
 * Based on example non-ext-ified  from: 
 * 
...
GlobalPostMessage  = {
     ...
     */
    request: function(requestedOptions) {
        var options = {
            success: function(x) {},
            failure: function(x) {
                throw x
        ...
        for (var option in requestedOptions){
            if(requestedOptions.hasOwnProperty(option)){
                options[option] = requestedOptions[option];
            }
        }
        ...
        //<debug>
        if (!key) {
            throw 'A key must be specified for a Post Message request.';
        }
        //</debug>
        ...
        if (timeout > 0) {
            request.timeout = setTimeout(function(){
                me.handleTimeout.apply(me, [request]);
            }, timeout);
        }
    ...
    destroyMessageListener: function() {
        ...
        if (me.listening) {
            delete me.listening;
            window.removeEventListener('message', function(args){
              me.onMessage.apply(me, [args])  
            });
        }
    }
